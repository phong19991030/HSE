...
@Service("Sys_new_1000ServiceImpl")
public class Sys_1000ServiceImpl extends AbstractService{
	...
	public List getMaintenanceCodeList(Map param) throws Exception{
		return ArrangeUtil.sortMapList(dao.getMaintenanceCodeList(param), "CODE", "UP_CD", "LEV");
	}
	...
	public Map insert(Map param) throws Exception {
		...
		/* 메뉴 추가 - TSST_MENU_MGT */
		int insert_menu_cnt = dao.insertMaintenanceCode(param);
		/* 메뉴 추가 결과 */
		result.put("INSERT_CODE_CNT", insert_menu_cnt > 0 ? 1 : 0);
		result.put("MAINTEN_CD", param.get("MAINTEN_CD"));
		return result;
	}
	...
	public Map update(Map param) throws Exception {
		Map result = new HashMap();
		/* 메뉴 수정 - TSST_MENU_MGT */
		int update_menu_cnt = dao.updateMaintenanceCode(param);
		/* 메뉴 수정 결과 */
	...
	public Map delete(Map param) throws Exception {
		...
		try {
			int delete_code_cnt = dao.deleteMaintenanceCode(param);
	...
	public Map checkDuplicateMaintenanceCode(Map param) throws Exception {
		return dao.checkDuplicateMaintenanceCode(param);
	}
}
