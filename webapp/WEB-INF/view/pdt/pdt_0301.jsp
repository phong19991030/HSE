...
<div class="container prediction-form">
	<div class="tit-wrap">
		<h2 class="heading3">
			<span class="txt">Prediction</span> <span class="version">Active power</span>
		</h2>
		<ul class="location">
			<li>Prediction</li>
			<li>HANGWON</li>
		...
		<div class="calendar-picker">
			<div class="calendar-wrap">
				...
				<div class="input-group">
					<label for="searchText" class="sr-only"></label> <input type="text"
						id="from_dt" class="datetimepicker" placeholder="Begin date"
						name="searchText" value="" style="width: 200px !important;">
					<span class="calendar-picker-btn"> <i class="xi-calendar"></i>
					</span>
				</div>
				<!-- /Data Picker -->
				<em class="hyphen"> <span class="sr-only">-</span>
				</em>
				<!-- Data Picker -->
				<div class="input-group">
					<label for="searchText" class="sr-only"></label> <input type="text"
						id="to_dt" class="datetimepicker" placeholder="End date"
						name="searchText" value="" style="width: 200px !important;">
					<span class="calendar-picker-btn"> <i class="xi-calendar"></i>
					</span>
				</div>
			...
			<span class="cal-btn-right"> <a class="refresh-btn2"> <i
					class="xi-refresh"> <span class="sr-only">refresh 버튼</span>
				</i>
			</a>
	...
	<div class="predict-tab">
		...
		<label class="tab_item" for="tab-forecast">Forecast</label> <input
			id="tab-analyze" type="radio" name="tab_item"> <label
			class="tab_item" for="tab-analyze">Analyze</label>
		...
		<div class="tab-content" id="forecast-content">
			...
			<div class="prediction-graph">
				<div class="tit-wrap">
					<strong class="heading6">Active power forecast for the next
						day</strong>
				</div>
				<div class="graph-wrap"
					style="width: 100%; height: 380px; background: #f4f4f4;">
					<div id="container1" style="width: 100%"></div>

					<div class="prediction-legend-wrap">
						<span class="legend-item1">Real data</span> <span
							class="legend-item2">Predictive data</span>
					</div>
				</div>
			</div>
			<!-- //graph -->
			...
			<div class="base_grid_table table-wrap">
				...
				<table>
					<caption>Active power forecast for the next 30 days</caption>
					<colgroup>
						<col style="width: 20%">
						<col style="width: 40%">
						<col style="width: 40%">
					...
					<thead>
						<tr>
							...
							<th scope="col">Real - Active power (kW)</th>
							<th scope="col">Predicted - Active power (kW)</th>
						</tr>
					</thead>
				</table>
		...
		<div class="tab-content" id="analyze-content">
			...
			<div class="prediction-graph">
				<div class="tit-wrap">
					<strong class="heading6">Active power forecast deviation
						from actual</strong>
				</div>
				<div class="graph-wrap"
					style="width: 100%; height: 380px; background: #f4f4f4;">
			...
			<div class="prediction-graph">
				<div class="tit-wrap">
					<strong class="heading6">Active power forecast accuracy</strong>
				</div>
				<div class="graph-wrap"
					style="width: 100%; height: 380px; background: #f4f4f4;">
					<div id="container3" style="width: 100%"></div>
					<div class="prediction-legend-wrap">
						<span class="legend-item3">Accuracy (%)</span> <span
							class="legend-item4">Error rate (%)</span>
					</div>
				</div>
			</div>
			<div class="base_grid_table table-wrap">
...
// 	$('.datepicker').setDatePicker('yy-mm-dd');
	...
	// datepicker 설정
// 	if('${SESS_USER.LANG_CODE}' == 'en'){
// 		$('.datetimepicker').setDateTimePicker('dd-mm-yy');	
// 	}
// 	else if('${SESS_USER.LANG_CODE}' == 'ko'){
...
// 		 return false
/////// 		example
		from_dt = '2021-02-28 00:00:00';
		to_dt = '2021-03-01 00:00:00';
		param = {FROM_DATE: from_dt, TO_DATE: to_dt};
	}else{
		$('#from_dt').val("");
		$('#to_dt').val("");
		alert('Data is not enough now!')
		from_dt = '2021-02-28 00:00:00';
		to_dt = '2021-03-01 00:00:00';
		param = {FROM_DATE: from_dt, TO_DATE: to_dt};
	}
...
// 	$('.table-wrap').hide()
	$.ajax({
		  url: 'http://34.64.201.18:9090/public/pdt_0301/getData.ajax',
// 		  url: 'http://localhost:9090/public/pdt_0301/getData.ajax',
		  type: 'GET',
		  data: param,
// 		  headers: {  
...
// 				var today = '2021-01-28';
				...
				for(var i = 0; i < dates.length; i++){
					...
					HIS[i] = historyValue && historyValue.length != 0? Number(historyValue[0].POWER.toFixed(2)): null;
						
					var predictValue  = data.PREDICT.filter(function(o){return o.DT == dates[i];});
					if(predictValue && predictValue.length != 0){
					  	PRE[i] = Number(predictValue[0].POWER.toFixed(2));
					  	if(index < 0){
					  		index = i;
					  	}
					  	if(PRE[i] != null && HIS[i] != null){
							...
							ERROR[i-index] = (PRE[i] == 0 && HIS[i] == 0) ? 0: (Math.abs((PRE[i] -  HIS[i])*100/HIS[i]) > 100)? 100: Number((Math.abs((PRE[i] -  HIS[i])*100/HIS[i])).toFixed(2));
							ACCURACY[i-index] = ERROR[i-index] == 100? 0: Number((100 - ERROR[i-index]).toFixed(2));
							if( ERROR[i - index] < 0){
								debugger;
							}
							console.log(dates[i]+ ' - ' + DIFF[i - index]+ ' - ' + ERROR[i - index]+ ' - ' + ACCURACY[i - index]);
					  	}
...
function dateRange(startDate, endDate, steps) {
  ...
  while (currentDate <= new Date(endDate.replace(' ', 'T'))) {
    dateArray.push((new Date(currentDate)).getFromFormat('yyyy-mm-dd hh:ii:ss'));
	...
	Highcharts.chart('container2', {
	    ...
	    tooltip: {
	         ...
	         formatter: function(){
	            ...
	            '<span  style="font-weight: 300;">'+ this.series.name +  '</span>:<span style="margin-left: 5px; font-weight: 600;"> ' + this.y + ' kW</span>' +
	                 '</div>';
	         }
	    },
	    series: [{
...
function drawTable2(data) {
	...
	var avg =  powers.length > 0?  Number(getAverage(powers).toFixed(2)): 0;
	var str = '';
	str+= '<tr><th>Min</th><td>'+min+' kW</td></tr>'
	+ '<tr><th>Max</th><td>'+max+' kW</td></tr>'
	+ '<tr><th>Average</th><td>'+avg+' kW</td></tr>';
