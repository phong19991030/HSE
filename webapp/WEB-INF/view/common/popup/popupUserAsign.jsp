...
var popupDrawgrid = function(formId,data){
	$('#popupGrid').setViewGrid({id:'popupGrid',type : 'crud', pinHeader : true,
		url:CTX+'/common/popup/getDataUserAsignPopup?GRADE='+grade +"&COAPVL=" + coapvl,  
		param : formId ,
		localData : data,
		data : data,  
		gridOptions : {height: 420, width:200, sortable:true, caption: '검색결과' , loadonce:true},
		colModels : [	             
     	   	{name:'<spring:message code='title.asign.emp_no.type' />', id :'EMP_NO', width:60},
   	   		{name:'<spring:message code='title.asign.emp_name.type' />', id :'NAME', width:60, align:'left'}, 
   	     	{name:'<spring:message code='title.asign.dept_no.type' />', id :'DEPT_NM', width:60, align:'left'},
     	    {name:'<spring:message code='title.asign.position.type' />', id :'PST_NM', width:80, align:'left'}
   	   	],
