...
var popupDrawgrid1 = function(formId,data){
	$('#popupGrid1').setViewGrid({
		...
		url:CTX+'/stm/stm_0101/getDataList01.ajax',
		//url:CTX+'${param.data_url}',
		param : formId,  
		localData: data,  
		modelName : 'RESULTLIST',
		gridOptions : { height:600,	sortable:false, caption: '검색결과', loadonce:true, rownumbers:false},
		colModels : [
   	   		{name:'코드명', id :'COMM_NM_TREE', width:350, align:'left'},
   	   		{name:'코드명_원본', id :'COMM_NM', hidden:true},
   	   		{name:'코드', id :'COMM_CD', width:150},
...
//1. 다이얼로그
function onSelectAllcommCddialog(rowid, iRow, iCol, rowData, target,callback, obj){
	onSelectAll(rowid, iRow, iCol, rowData, target,callback, obj);
}

//2. 팝업
function onSelectAllcommCdpopup(rowid, iRow, iCol, rowData, target,callback, obj){
...
//그리드의 데이터를 모두 가지고 넘어가야할 경우
//1. 다이얼로그
function onSelectGridAllcommCddialog(rowid, iRow, iCol, rowData, target,callback, obj){
	onSelectGridAll(rowid, iRow, iCol, rowData, target,callback, obj);
}

//2. 팝업
function onSelectGridAllcommCdpopup(rowid, iRow, iCol, rowData, target,callback, obj){
