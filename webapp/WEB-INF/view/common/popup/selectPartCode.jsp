...
var drawgrid_part = function(formId,data){
	$('#gridPart').setViewGrid({
		...
		displayState : false ,	
		pinHeader : false, 			
		url:CTX+'/common/popup/popupPartCode/getDataList.ajax?CODE='+code,
		param : formId,  
		localData: data,  
...
function setStyle(){
	var grid = $('#gridPart').getViewGridWrap();
	var check = $('#PART').val();
	if(check!=null && check!=''){
		...
		$.each(rowList, function(i, obj){
			...
			var element = $obj[0].outerHTML;
	   		...
	   		var rowData = $('#table_gridPart').getRowData(rowId);
...
function treeCallback(grid) {
	...
	<a2m:searchbox script="drawgrid_part" >  
