...
(function(window){
  ...
  /*this.B = c || null;*/
	...
	Search.prototype.autoSet = function(){
		...
		this.A = $('select#SEARCH_PAYMENT_STATUS').val() || null;
...
function safety_0600() {
	...
	$('input#SEARCH_CRITERIA_ALL').keypress(function(e){
		if(e.keyCode === 13){
			...
			// 검색 setting
			//_search.autoSet().set('A',null).resetForm('select#SEARCH_PAYMENT_STATUS');
			_search.autoSet();
			onSearch();
		}
	});
	...
	$('button#REGISTER_BTN').click(function(){
		window.location = CTX + '/sft/sft_0601/registerForm';
	});
	onSearch();
}
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/sft/sft_0601/getData.ajax', param);
	if(data.LIST.length==0 && param.PAGE>1){
    param.PAGE--;
    data = _sys.mariaDB.getData(CTX + '/sft/sft_0601/getData.ajax', param);
  }
...
function makeList(data) {
	...
	if(list.length > 0) {
      ...
      +`</td>`
      +`<td>`
        +castEmptyData(e.INVESTOR)
      +`</td>`
      +`<td>`
        +`${castEmptyData(e.CSO_NAME)}, ${castEmptyData(e.FIELD_AGENT_NAME)}, ${castEmptyData(e.SAFETY_MANAGER_NAME)}, ${castEmptyData(e.MATERIAL_MANAGER_NAME)}, ${castEmptyData(e.SITE_MANAGER_NAME)}`
      +`</td>`
      +`</tr>`;
			...
			sample = $(sample).css('cursor', 'pointer').click(function(event) {
				if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
					...
					window.location = CTX + '/sft/sft_0601/detailForm?SAFE_MGT_ORG_ID=' + info.SAFE_MGT_ORG_ID;
				}
			});
		...
		$('tbody#ROW_LIST').append('<tr><td colspan="5" class="NO_DATA">No Data</td><tr>');
	}
};
