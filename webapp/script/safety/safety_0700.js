...
(function(window){
	...
	Search.prototype.autoSet = function(){
		...
		this.A = $('select#SEARCH_STATUS').val() || null;
		this.B = $('input#SEARCH_START_TIME').val() || null;
    this.C = $('input#SEARCH_END_TIME').val() || null;
    this.D = $('input#SEARCH_PAYMENT_STATUS').val() || null;
		return this;
	} 
	...
	Search.prototype.resetForm = function(target) {
      ...
      $('input#SEARCH_PAYMENT_STATUS').val('');
		}
		return this;
	}
  ...
  createSearchStatus();
	...
	$('input#SEARCH_CRITERIA_ALL').keypress(function(e){
		if(e.keyCode === 13){
			...
			// 검색 setting
			// _search.autoSet().set('A',null).set('B',null).set('C',null).set('D',null).
      // resetForm('select#SEARCH_STATUS').resetForm('input#SEARCH_START_TIME').
      // resetForm('input#SEARCH_END_TIME'). resetForm('input#SEARCH_PAYMENT_STATUS');
      _search.autoSet();
	...
	$('button#REGISTER_BTN').click(function(){
		window.location = CTX + '/sft/sft_0701/registerForm';
	});
	onSearch();
}
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/sft/sft_0701/getData.ajax', param);
	if(data.LIST.length==0 && param.PAGE>1){
    param.PAGE--;
    data = _sys.mariaDB.getData(CTX + '/sft/sft_0701/getData.ajax', param);
  }
...
function makeList(data){
  ...
  list.forEach((e) => {
    ...
    +`	<td class="txt-left">`
    +`	  ${castEmptyData(e.PROJECT_NAME)}`
    +`	</td>`
    +`	<td>`
    +	  castEmptyData(e.DOC_NO)
    +`	</td>`
    +`	<td>`
    +`	  ${castEmptyData(e.CHECKER_NAME)}`
    +`	</td>`
    +`	<td>`
    +`	  ${castEmptyData(e.CHECK_DATE)}`
    +`	</td>`
      /* D : state-style`
        1. 완료 - done: state-style state1`
        2. 점검중 - checking : state-style state2`
        3. 미점검 - uncheck: state-style state4`
      */
    + checkStatus(e.COMMON_CHECK_COMM_CD)
    + checkStatus(e.HEALTH_CHECK_COMM_CD)
    + checkStatus(e.SAFETY_CHECK_COMM_CD)
    + checkStatus(e.WORKING_CHECK_COMM_CD)
    + checkStatus(e.ENVIROMENT_CHECK_COMM_CD)
    +`</tr>`;
    ...
    sample = $(sample).css('cursor', 'pointer').click(function(event) {
      if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
        ...
        window.location = CTX + '/sft/sft_0701/detailForm?SAFE_CHECK_ID=' + info.SAFE_CHECK_ID;
      }
    });
...
function checkStatus(status){
  switch (status) {
    case "CHECK_STATUS-1": // Inspection Completed
      return `<td>`
            +`	  <span class="state-style state1">완료</span>`
            +`</td>`;
    case "CHECK_STATUS-2": // Under Inspection
      return `<td>`
            +`	  <span class="state-style state2">점검중</span>`
            +`</td>`;
    case "CHECK_STATUS-3": // Unchecked
      return `<td>`
            +`	  <span class="state-style state4">미점검</span>`
            +`</td>`;  
    default:
      return `<td>`
...
function createSearchStatus(){
  ...
  check_list.forEach( e =>{
    var option = `<option value="${e.COMM_CD}">${e.NAME}</option>`;

    $('select#SEARCH_STATUS').append(option);
  });
}
