...
(function(window){
  ...
  /*this.B = c || null;*/
	...
	Search.prototype.autoSet = function(){
		this.ALL = $('input#id_search_txt').val() || null;
		this.A = $('select#id_search_payment').val() || null;
	...
	Search.prototype.getParam = function() {
		var param = {};
		if(this.ALL) param.all = this.ALL;
		if(this.A) param.SEARCH_PAYMENT_STATUS = this.A;
		/*if(this.B) param.SEARCH_ROLE_NM = this.B*/;
		return param;
	}
...
function safety_0400() {
	...
	$('input#id_search_txt').keypress(function(e){
		if(e.keyCode === 13){
			...
			// 검색 setting
			//_search.autoSet().set('A',null).resetForm('select#id_search_payment');
			_search.autoSet();
			onSearch();
		}
	});
	...
	$('button#SEARCH_BTN').click(function(){
		...
		//검색 setting
		//_search.autoSet().set('ALL',null).resetForm('input#id_search_txt');
    _search.autoSet()
	...
	$('button#REGISTER_BTN').click(function(){
		window.location = CTX + '/sft/sft_0401/registerForm';
	});
	onSearch();
}
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/sft/sft_0401/getData.ajax', param);
  if(data.LIST.length==0 && param.PAGE>1){
    param.PAGE--;
    data = _sys.mariaDB.getData(CTX + '/sft/sft_0401/getData.ajax', param);
  }
...
function makeList(data) {
	...
	if(list.length > 0) {
		list.forEach((e) => {
			...
			var sample = 
				'<tr>'
					...
					+ '	<td>' + castEmptyData(e.WRITER_NAME) + '</td>' 
					+ '	<td>' + castEmptyData(e.EMERGENCY_PHONE) + '</td>' 
					+ '	<td>' + convertArrStr(e.EXPECTED_EMERGENCY) + '</td>' 
					+ '	<td class="txt-left">' + convertArrStr(e.PROCEDURE_ACTION) + '</td>' 
					+ '	<td>' + e.EVACUATION_ROUTE + '</td>' 
				+ '</tr>';
			...
			sample = $(sample).css('cursor', 'pointer').click(function(event) {
				if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
					...
					window.location = CTX + '/sft/sft_0401/detailForm?ERP_ID=' + info.ERP_ID;
				}
			});
		...
		$('tbody#ROW_LIST').append('<tr><td colspan="7" class="NO_DATA">No Data</td><tr>');
	}
};
