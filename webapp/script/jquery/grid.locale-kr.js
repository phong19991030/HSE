...
$.extend($.jgrid,{
	defaults : {
		recordtext: "총{2}개 중 ({0}부터 ~ {1}까지)",
		emptyrecords: "표시할 데이터가 없습니다",
//		loadtext: "데이터를 불러오는 중입니다.",
		loadtext: "",
		pgtext : "페이지 {0} / {1}"
	},
	search : {
...
//트리 동적 생성 시 id외에 파라미터를 추가적으로 부여하기 위해 getDynamicTreeData를 호출해 준다.
$(document).ready(function() {
	//트리그리드에 동적 파라미터 관리가 필요한 경우에만(구현한 경우에만)
	if(typeof(getDynamicTreeData) != 'undefined'){
		var originExpandNode = $.fn.jqGrid.expandNode;
		//expandNode 메서드를 확장하여 getDynamicTreeData를 호출해 준다(파라미터는 = rowData)
		$.jgrid.extend({
			expandNode : function(rc){
				getDynamicTreeData(rc);
				return originExpandNode.call(this, rc);	
			}
		});
	}
});	
