...
(function(window){
	...
	Search.prototype.autoSet = function(){
		...
		this.B = $('select#SEARCH_CRITERIA_B option:selected').val() || null;
		return this;
	} 
	...
	Search.prototype.resetForm = function(target) {
		if(target) {
			...
			$("select#SEARCH_CRITERIA_B option:eq(0)").prop("selected", true);
			$("select#SEARCH_CRITERIA_B").siblings('label').text($("select#SEARCH_CRITERIA_B option:eq(0)").text());
		}
		return this;
	}
	...
	Search.prototype.getParam = function() {
		...
		if(this.A) param.SEARCH_COMPANY_NM = this.A;
		if(this.B) param.SEARCH_CLS = this.B;
		return param;
	}
...
function sys0500() {
	...
	$('a#REGISTER_BTN').click(function(){
		window.location = ctx + '/sys_new/sys_0500/registerForm';
	});
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/sys_new/sys_0500/getData.ajax', param);
	console.log(data);
...
function makeList(data) {
	...
	list.forEach((e) => {
		...
		var sample = 
			...
			+ '	<td>' + cls[e.CLS] + '</td>'
			+ '	<td>' + e.COMPANY_NM + '</td>'
			+ '	<td><a href="javascript:void(0);">[' + e.USER_CNT + ']</a></td>'
			+ '</tr>';
		...
		sample = $(sample).css('cursor', 'pointer').click(function(event) {
			if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
				...
				window.location = CTX + '/sys_new/sys_0500/detailForm?COMPANY_ID=' + info.COMPANY_ID;
			}
		});
		
		$(sample).find('a').click(function() {
			var info = $(this).parents('tr').prop('info');
			window.location = CTX + '/sys_new/sys_0300/list?COMPANY_ID=' + info.COMPANY_ID;
		});
	...
	// 데이터 0개 일 경우 
	if(list.length === 0) $('tbody#ROW_LIST').append('<tr><td colspan="4" class="NO_DATA">No Data</td><tr>');
};
