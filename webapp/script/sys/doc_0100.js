...
(function(window){
	...
	Search.prototype.getParam = function() {
		var param = {};
		if(this.ALL) param.DOC_NAME = this.ALL;
		if(this.A) param.DOC_GROUP = this.A;
		if(this.B) param.INS_DATE = this.B;
		return param;
	}
...
function sysDoc() {
	...
	$('a#REGISTER_BTN').click(function(){
		window.location = CTX + '/sys_new/doc_0100/registerForm';
	});
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/sys_new/doc_0100/getData.ajax', param);
...
function onGetGroup(){
	var docGroups = _sys.mariaDB.getData(CTX + '/sys_new/doc_0100/getGroup.ajax');
...
function makeList(data) {
	...
	list.forEach((e) => {
		...
		var sample = 
			...
			+ '	<td>' + e.DOC_NAME + '</td>'
			+ '	<td>' + e.INS_DATE + '</td>'
			+ '	<td>' + (e.INS_USER_NAME ? e.INS_USER_NAME : "--") + '</td>'
			+ '</tr>';
		...
		sample = $(sample).css('cursor', 'pointer').click(function(event) {
			if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
				...
				window.location = CTX + '/sys_new/doc_0100/detailForm?DOC_ID=' + info.DOC_ID;
			}
		});
