...
(function(window){
	...
	Search.prototype.autoSet = function(){
		...
		this.A = $('input#SEARCH_MENU_A').val() || null;
...
function sys0600() {
	...
	$('input#SEARCH_MENU_ALL').keypress(function(e){
		if(e.keyCode === 13){
			...
			// 검색 setting
			//_search.autoSet().set('A',null).set('B',null).resetForm('input#SEARCH_MENU_A').resetForm('input#SEARCH_MENU_B');
			_search.autoSet();
			onSearch();
		}
	});
	...
	$('button#SEARCH_BTN').click(function(){
		...
		//검색 setting
		//_search.autoSet().set('ALL',null).resetForm('input#SEARCH_MENU_ALL');
		_search.autoSet()
		//page 초기화
	...
	$('button#REGISTER_BTN').click(function(){
		...
		window.location = CTX + '/sys_new/sys_0600/registerForm';
	});
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/sys_new/sys_0600/getData.ajax', param);
	...
	list.forEach((e) => {
		...
		sample = $(sample).css('cursor', 'pointer').click(function(event) {
			...
			if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
				...
				window.location = CTX + '/sys_new/sys_0600/detailForm?ROLE_ID=' + info.ROLE_ID;
			}
		});
