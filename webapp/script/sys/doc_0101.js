...
function sysDocRegister() {
    ...
    $('span#DELETE_BTN').click(function() {
		...
		if(result.IS_DELETE) {
			...
			window.location = CTX + '/sys_new/doc_0100/list';
		}
		// Exception 발생
    ...
    var initGroup = getParameterByName("DOC_GROUP");
    if(initGroup){
      
        $('input#DOC_GROUP').val(initGroup);
    }
...
function doSave(param){
    ...
    if (data.INSERT_DOC_CNT > 0 || data.UPDATE_DOC_CNT > 0) {
        ...
        window.location = CTX + '/sys_new/doc_0100/detailForm?DOC_ID=' + data.DOC_ID;
    }
    else {
...
function modifyInit() {
    // 데이터 조회
    var data = _sys.mariaDB.getData(CTX + '/sys_new/doc_0100/detailForm/getDetailInfo.ajax', {
        DOC_ID: $('input#DOC_ID').val(),
    });
    // 데이터 없을 경우 return 
    if (!data) return;
    docData = data;
    $("#imgSrc").attr("src", CTX + '/util/upload/imageView/' + docData.FILE_ID);
