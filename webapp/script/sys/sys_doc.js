...
function sysDoc() {
	...
	$('a#REGISTER_BTN').click(function(){
		window.location = CTX + '/sys_new/doc_0001/registerForm';
	});
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/sys_new/doc_0001/getData.ajax', param);
...
function onGetGroup(){
	var docGroups = _sys.mariaDB.getData(CTX + '/sys_new/doc_0001/getGroup.ajax');
}
	...
	list.forEach((e) => {
		...
		sample = $(sample).css('cursor', 'pointer').click(function(event) {
			if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
				...
				window.location = CTX + '/sys_new/doc_0001/detailForm?DOC_ID=' + info.DOC_ID;
			}
		});
