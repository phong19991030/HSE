...
(function(window){
	...
	Search.prototype.getParam = function() {
		...
		if(this.A) param.SEARCH_GROUP_CODE = this.A;
		if(this.B) param.SEARCH_ALARM_NM = this.B;
		return param;
	}
...
function sys0900() {
	...
	$('a#REGISTER_BTN').click(function(){
		window.location = ctx + '/sys_new/sys_0900/registerForm';
	});
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/sys_new/sys_0900/getData.ajax', param);
	console.log(data);
...
function makeList(data) {
	...
	list.forEach((e) => {
		...
		var sample = 
			...
			+ '	<td>' + e.ALARM_GROUP_CODE + '</td>'
			+ '	<td>' + e.MANUFACTURER_NM + '</td>'
			+ '	<td>' + e.MODEL_NM + '</td>'
			+ '	<td>' + e.ALARM_CNT + '</td>'
			+ '	<td>' + e.ALARM_NM + '</td>'
		...
		sample = $(sample).css('cursor', 'pointer').click(function(event) {
			if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
				...
				window.location = CTX + '/sys_new/sys_0900/detailForm?WT_ALARM_GR_ID=' + info.WT_ALARM_GR_ID;
			}
		});
