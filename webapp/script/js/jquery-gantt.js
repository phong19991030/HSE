...
var Gantt = function () {
  ...
  createClass(Gantt, [{
    ...
    value: function renderGrid(num) {
      ...
      var startDate = new Date(this.config.startDate.getTime());
	  var ngayHn = new Date();	  
      var curDate = new Date(ngayHn.getFullYear(), ngayHn.getMonth(), ngayHn.getDate());
      for (var i = 0; i <= num; i += 1) {
        var templateGridCol = $$1('<div>', { class: 'gantt-grid-col', width: this.config.cellWidth, height: totalHeight });
        ...
        if (this.isEqual(startDate, curDate)) {
          templateGridCol.css('border-right', '5px solid rgb(192, 57, 43)');
        }
        templateGridCols.append(templateGridCol);
        startDate.setDate(startDate.getDate() + 1);
      }
...
var percen= percent(el.startdate,el.enddate,new Date());
        ...
        templateEventRow.append(templateEvent.css('left', left).append(eventBlock).append(eventDesc)).css('height', this.config.cellHeight);
    ...
    value: function tooltipView(data) {
      ...
      var html = this.format(template, data.image, data.title);
      return html;
    }
  }, {
    key: 'tooltipHover',
...
var date = new Date(str);
	if(Date.parse(date)>Date.parse(end)){
		return "100";
	}else if(Date.parse(date)<Date.parse(start)){
		return "0";
	}else{
		var num = (Date.parse(end)-Date.parse(start))/86400000+1;		
		var pass = (Date.parse(date)-Date.parse(start))/86400000+1;
		var result = (pass*100/num).toFixed(0);
		return result;
	}
}
