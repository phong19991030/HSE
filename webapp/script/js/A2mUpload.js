...
var readConfigFiles = function (context_path) {
    ...
    if (jsLocation) {
        ...
        $.ajax({
            ...
            async: false,
            success: function (data, textStatus, jqXHR) {
                if(typeof getDesignDefaultOpt  === 'function'){
                    designDefault = getDesignDefaultOpt(context_path);
                }
            },
            complete: function () {
...
//		comMessage = message;
//	}else{
//		swal('ì„¤ì •íŒŒì¼ì„ ì°¾ì„ ìˆ˜ê°€ ì—†ìŠµë‹ˆë‹¤.')
//	}
    }
}

readConfigFiles(context_path);
...
(function ($) {
    ...
    $.fn.A2mUpload = function (opts) {
        ...
        var A2mDms = function (opts) {
        	// console.log("A2mDms = function (opts)");
        	// console.log(opts);
            this.version = 1.0;
            ...
            $context.data = [],
                ...
                $context.reset = function () {
                    try {
                    	...
                    	//hieuph
                    	$context.a2mDmsUI.isComplete = false;
                    	//end
                    ...
                    try {
                        if (idx != -1) {
                            ...
                            if (file.isUploaded){
                            	...
                            	$context.lfu.removeFileByUIdentifier(String(file.uniqueIdentifier));
                            }
                            // End modify
                ...
                $context.removeTempleFiles = function () {
                    try {
                        $context.lfu.files = [];
                    }catch (e){
                        $context.lfu.developLogger.logger('e', 'removeTempleFiles', e);
                    }
                },
            ...
            //hieuph: delete file in hard disk
                $context.deleteFileInHardDisk = function () {
                	try {
                        var files = $context.willBeDeletedArray;
                        var deleteData = [];	
                        $.each(files, function (index, file) {
                             deleteData.push(file);
                        });
                ...
                $context.deleteFilesFromCache = function () {
                	try {
                        var files = $context.willBeDeletedArray;
                        $context.willBeDeletedArray = [];
                        return files;
            ...
            $context.defaults = {
                ...
                pauseImageUrl: opts.contextPath + "/images/pause.png",
                resumeImageUrl: opts.contextPath + "/images/resume.png",
                closeImageUrl: opts.contextPath + "/images/close.png",
                blockPolicy: 'black',
                blackExtension: [],
                whiteExtension: [],
            ...
            $context.init = function () {
                ...
                // a2mDmsUI groupID / form ê¸°ë°˜ ìƒì„±ë²„ì „
                ...
                //console.log(" $context.lfu = new LargeFileUpload");
                $context.lfu = new LargeFileUpload({
                    ...
                    method: "octet",
                    query: data,
                    generateUniqueIdentifier: $context.getOpt('generateUniqueIdentifier')
                });
...
//     			          $('.largefileupload-drop').show(); - JM
            	...
            	$context.lfu.opts.dom.on('click', '.largefileupload-file-name', function() {
                	$(this).parents('.largefile-layout-form').data('dms').downloadFileByUIdentifier($(this).siblings('input[type=checkbox]').val());
                });
            ...
            var fileForm = function (file, obj) {
                ...
                if(design.component == undefined){
                    design = new Design(opts).defaults;
                }
                var size = 0;
...
//                        onclick: downJs
                  ...
                  if($context.getOpt('useAlias')) {
	                    var $aliastd = $("<td>", {
	                        class: design.component.fileList.columns.class,
	                        style: design.component.fileList.columns.css + "width:" + (design.component.fileList.width[0].indexOf("%") == -1 ? design.component.fileList.width[0] :
	                            obj.find(".largefileupload-foreground").width() * (design.component.fileList.width[0].replace("%", "") * 0.01))
	                        + "px;height:" + (design.component.fileList.height[0].indexOf("%") == -1 ? design.component.fileList.height[0] :
	                            listheight * (design.component.fileList.height[0].replace("%", "") * 0.01)) + "px;"
	                    }).appendTo($largefileupload_file);
	                    var $aliasinput = $("<input>", {
	                    	type: 'text',
	                        class: "largefileupload-file-alias-" + file.uniqueIdentifier,
	                        value: file.isUploaded ? file.alias : file.fileName,
                    		readonly: file.isUploaded
	                    }).appendTo($aliastd);
                	}
...
//				fileAction = fileAction + '<a href="#" onclick="a2mDmsUI.deleteFileByUIdentifier(\''+file.uniqueIdentifier+'\')" class="btn btn-warning btn-xs progress-delete-link-'+file.uniqueIdentifier+'" style="display:none;">Delete</a>';
                    ...
                    if ($context.getOpt('controlType') == 'UPLOADBOX') {
                        ...
                        var imgdown = $("<img>", {
                            ...
                            style: "padding-right:0px;opacity:0.6;",
                            alt: "선택된 파일 다운로드"
                ...
                if ($context.getOpt('dropAreaView') === false) {
                    ...
                    var a = $("<a>", {
                        ...
                        html: '<img src="' + (design.component.Toolbar.addbutton === undefined ? $context.defaults.design.component.Toolbar.addbutton.img :
                            design.component.Toolbar.addbutton.img) + '" style=" padding-right:0px" alt="파일 추가">'
...
//                    }).appendTo($tr);
                    ...
                    var $colName = $("<col>", {
                    	style: "width:" + design.component.fileList.width[0],
                    }).appendTo($colgroup);
                    ...
                    var $colSize = $("<col>", {
                    	style: "width:" + design.component.fileList.width[1],
                    }).appendTo($colgroup);
                        ...
                        swal(comMessage.DeleteConfirm,
                            function (isConfirm) {
                                ...
                                $.each(arr, function (i, s) {
                                    ...
                                    if(file != undefined){
                                    	$context.willBeDeletedArray.push(file);                                    	
                                    }
                                    
                                    var onDeleteComplete = $context.getOpt('onDeleteComplete');
                                    if(onDeleteComplete) {
                                    	onDeleteComplete($context, file);
                                    }
//                                    if (file.isUploaded) {
...
//                        });
            ...
            $context.deleteFiles = function (files, events) {
                ...
                try {
                    ...
                    $.ajax({
                        ...
                        suppressErrors: false,
                        success: function (data, textStatus, jqXHR) {
                        	/*console.log("delete files sucessfully at : ");
                        	console.log(data);
                        	console.log(textStatus);
                        	console.log(jqXHR);
                        	console.log("end delete files sucessfully at : ");*/
                            $context.lfu.developLogger.logger('i', "delete success");
                        }, complete: function () {
                        	/*console.log("delete files completely at : ");*/
                        	...
                        	/*console.log("delete files error at : ");*/
        ...
        if (opts) {
            ...
            //var  url = a2mdms.getOpt('transmitDirection').foldercheck === undefined ? a2mdms.defaults.transmitDirection.foldercheck:a2mdms.getPath(a2mdms.getOpt('transmitDirection').foldercheck);
            //console.log("url licenseKey = " + url + "with licenseKey= " + a2mdms.getOpt('licenseKey'));
            $.ajax({
                ...
                async: false,
                success: function (data, textStatus, jqXHR) {
                	//console.log("check licenseKey");
                	
                	//hieuph
                    /*if (data.match(/true/)) {
                    	console.log("---check licenseKey: true");
                        a2mdms.init();
                        jQuery.data($a2mUpload, 'dms', a2mdms);
                        $a2mUpload.data('dms', a2mdms);
                        $a2mUpload.addClass('largefile-layout-form');
                    } else if(data.match(/false/)){
                    	console.log("---check licenseKey: false");
                        a2mdms.init();
                        a2mdms.lfu.developLogger.logger('e','ì—…ë¡œë“œ í•  í´ë”ë¥¼ ì°¾ì„ ìˆ˜ ì—†ìŠµë‹ˆë‹¤. ê²½ë¡œë¥¼ ë‹¤ì‹œ í™•ì¸í•´ì£¼ì„¸ìš”.');
                        a2mdms.destroy();
                    }*/
                ...
                error: function (request, status, error, event) {
                	//console.log("check licenseKey error");
                    try {
...
//			this.opts.dom.hide();
        ...
        if(isNeroSupport()) {
        	...
        	if(this.mA2mDms.length > 0) {
                ...
                var successFunction = function(data, statusText, xhr, form){
                    //ì„±ê³µí›„ ì„œë²„ì—ì„œ ë°›ì€ ë°ì´í„° ì²˜ë¦¬
    ...
    this.complete = function (pA2mDms, e) {
        ...
        $.each(pA2mDms.data, function (i, d) {
            ...
            var alias = $('.largefileupload-file-alias-' + pA2mDms.data[i].uniqueIdentifier);
            if(alias.length > 0) {
                pA2mDms.data[i].alias = $('.largefileupload-file-alias-' + pA2mDms.data[i].uniqueIdentifier).val();
            }
        });
...
//		        	  	     pA2mDms.lfu.getOpt('dom').parents('form').submit();
           ...
           var uploadObject;
            ...
            if(obj.mA2mDms.length > 0){
            	...
            	uploadObject = obj.mA2mDms[0];
            }
            
            if($('.a2m-dms-dialog').data('dmsUI') != null ){
...
//				fileAction = fileAction + '<a href="#" onclick="a2mDmsUI.deleteFileByUIdentifier(\''+file.uniqueIdentifier+'\','+pLfu.largeFileUploadId+')" class="btn btn-warning btn-xs progress-delete-link-'+file.uniqueIdentifier+'" style="display:none;">Delete</a>';
    ...
    this.buildUploadBox = function () {
        ...
        var design = $context.getOpt('design');
        if(design.uploadPopup == undefined){
            design = new Design(opts);
            isUseDefault = true;
        }
        if ($('#a2m-dms-ui').length == 0) {
            ...
            var res= ""; 
            if(isUseDefault){
                res = '<div class="a2m-dms-upload-titlebar"><img src="' + design.defaults.component.Toolbar.uploadPopup.uploadImg + '" style="padding-right: 0px;" alt="ì—…ë¡œë“œ ì°½ ì•„ì´ì½˜"/><b>íŒŒì¼ ì—…ë¡œë“œ</b></div>';
            }
            else{
                res = '<div class="a2m-dms-upload-titlebar"><img src="' + design.uploadPopup.uploadImg + '" style="padding-right: 0px;" alt="ì—…ë¡œë“œ ì°½ ì•„ì´ì½˜"/><b>íŒŒì¼ ì—…ë¡œë“œ</b></div>';
            }
            ...
            res = res + '<div id="a2m-dms-uploadbox" > <div id="a2m-dms-frame"> '
                ...
                + '<caption>파일의 선택정보, Name, Size, Progess을 확인 할 수 있습니다.</caption>'
                // + '<thead role="rowgroup"> '
                // + '<caption>íŒŒì¼ì˜ ì„ íƒì •ë³´, Name, Size, Progessì„ í™•ì¸ í•  ìˆ˜ ìžˆìŠµë‹ˆë‹¤.</caption>'
                // + '<tr class="row"> '
...
var Design = function (opts) {
    ...
    this.version = '1.0';
    $context = this;
    //console.log("Design : $context.opts.contextPath = " + contextPath + "/images/file.png/");
    $context.defaults = {
        themes: "basic",
        motion: {
            type: "basic",
        ...
        component: {
            ...
            dropbox: {
                ...
                class: "",
                coverImgText: {
                    img:  context_path + "/images/file.png",
            ...
            Toolbar: {
                ...
                addbutton: {
                    ...
                    width: "13%",
                    height: "100%",
                    img: context_path + "/images/plus.png",
                    top: "",
                    left: "",
                    class: "",
                ...
                delbutton: {
                    ...
                    width: "",
                    height: "",
                    img: context_path + "/images/delete.png",
                    top: "",
                    left: "",
                    class: "",
                ...
                downbutton: {
                    ...
                    width: "",
                    height: "",
                    img: context_path + "/images/download.png",
                    top: "",
                    left: "",
                    class: "",
                ...
                uploadPopup:{
                    uploadImg: context_path +  "/images/upload.png",
		            deleteImg: context_path + "/images/close.png"
                }
            }
        }
    };
