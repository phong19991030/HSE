 ...
 */
if (File.prototype.slice === undefined) {
...
var WebSocketFileTransfer = new Class({
	...
	onMessage: function(event) {
		...
		var resf = self.parseResponse(event.data);
		response = resf.PAYLOAD;
		...
		if( response.type == 'AUTH' ) {
			...
			self.options.sess_key = response.sess_key;
			self.filepath = response.filepath;
			self.savename = response.savename;
			self.ext = response.ext;
			//loader_dict[response.sess_key] = self.options.loader;
			...
			if( lastBlock ) {
				...
				setTimeout(function() {
					...
					if( typeof(self.options.success) == 'function' ) {
						var param = {};
						param.name = self.file.name;
						param.orinPath = self.file.webkitRelativePath;
						param.size = self.file.size;
						param.type = self.file.type;
						param.path = self.filepath;
						param.savename = self.savename;
						param.ext = self.ext;
						self.options.success(e, param);
					}
				}, 0);
	...
	sendSlice: function(data) {
		var infos = {
				...
				'size': this.file.size,
				'type': this.options.type,
				'sess_key': this.options.sess_key,
				'data': data
			};
		this.socket.send('DATA: '+JSON.encode(infos));
