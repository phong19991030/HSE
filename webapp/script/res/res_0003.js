...
(function(window){
	...
	Search.prototype.autoSet = function(){
		...
		this.B = $('select#SEARCH_CRITERIA_B').val() || null;
	...
	Search.prototype.getParam = function() {
		...
		if(this.A) param.SEARCH_DATE_OF_ISSUE = this.A;
		if(this.B) param.SEARCH_HAZARDOUS_TYPE = this.B;
    if(this.C) param.SEARCH_STORAGE_PLAN = this.C;
...
function res_0003() {
	...
	$('input#SEARCH_CRITERIA_ALL').keypress(function(e){
		if(e.keyCode === 13){
          ...
          //.resetForm('input#SEARCH_CRITERIA_A').resetForm('select#SEARCH_CRITERIA_B')
	...
	$('button#REGISTER_BTN').click(function(){
		window.location = CTX + '/res/res_0003/registerForm';
	});
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/res/res_0003/getData.ajax', param);
	if(data.LIST.length==0 && param.PAGE>1){
    param.PAGE--;
    data = _sys.mariaDB.getData(CTX + '/res/res_0003/getData.ajax', param);
  }
...
function makeList(data) {
	...
	// COMPANY 분류
      ...
      +`	</td>`
      +`	<td>${castEmptyData(e.DATE_OF_ISSUE)}</td>`
      +`	<td>${castEmptyData(e.MANAGER_NAME)}</td>`
      +`	<td>${castEmptyData(e.HAZARDOUS_TYPE)}</td>`
      +`	<td>${castEmptyData(e.AMOUNT_HAZARDOUS)} L</td>`
      +`	<td>${castEmptyData(e.STORAGE_PLAN)}</td>`
      +`	<td>${castEmptyData(e.CAUTION)}</td>`
      +`	<td>`
      // +`	  <!-- D : state-color`
      ...
      +`</tr>`;
			...
			sample = $(sample).css('cursor', 'pointer').click(function(event) {
				if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
					...
					window.location = CTX + '/res/res_0003/detailForm?HAZARDOUS_ID=' + info.HAZARDOUS_ID;
				}
			});
