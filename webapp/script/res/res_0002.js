...
(function(window){
	...
	Search.prototype.autoSet = function(){
		...
		this.A = $('select#SEARCH_CRITERIA_A').val() || null;
		this.B = $('input#SEARCH_CRITERIA_B').val() || null;
    this.C = $('select#SEARCH_CRITERIA_C').val() || null;
    this.D = $('select#SEARCH_CRITERIA_D').val() || null;
		return this;
	} 
	...
	Search.prototype.resetForm = function(target) {
		if(target) {
			...
			$('select#SEARCH_CRITERIA_A').val('');
      $('input#SEARCH_CRITERIA_B').val('');
			$('select#SEARCH_CRITERIA_C').val('');
      $('select#SEARCH_CRITERIA_D').val('');
		}
		return this;
	}
	...
	Search.prototype.getParam = function() {
		...
		if(this.A) param.SEARCH_WASTE_TYPE = this.A;
		if(this.B) param.SEARCH_DISPOSAL_DATE = this.B;
    if(this.C) param.SEARCH_LICENSE = this.C;
    if(this.D) param.SEARCH_PAYMENT = this.D;
		return param;
	}
...
function res_0002() {
	...
	$('input#SEARCH_CRITERIA_ALL').keypress(function(e){
      ...
      //     .resetForm('select#SEARCH_CRITERIA_A').resetForm('input#SEARCH_CRITERIA_B')
      //     .resetForm('select#SEARCH_CRITERIA_C') .resetForm('select#SEARCH_CRITERIA_D');
          _search.autoSet();
			onSearch();
		}
	});
	...
	$('button#REGISTER_BTN').click(function(){
		window.location = CTX + '/res/res_0002/registerForm';
	});
	onSearch();
}
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/res/res_0002/getData.ajax', param);
	if(data.LIST.length==0 && param.PAGE>1){
    param.PAGE--;
    data = _sys.mariaDB.getData(CTX + '/res/res_0002/getData.ajax', param);
  }
...
function makeList(data) {
	...
	// COMPANY 분류
      ...
      +`	<td class="txt-left">`
      +`	  <p>${castEmptyData(e.PROJECT_NAME)}</p>`
      +`	</td>`
      +`	<td>${castEmptyData(e.DISPOSAL_DATE)}</td>`
      +`	<td>${castEmptyData(e.MANAGER_NAME)}</td>`
      +`	<td>${castEmptyData(e.WASTE_TYPE)}</td>`
      +`	<td>${castEmptyData(e.WASTE_GENERATION)}</td>`
      +`	<td>${castEmptyData(e.SELF_THROUGHPUT)}</td>`
      +`	<td>${castEmptyData(e.CONSIGNMENT_THROUGHPUT)}</td>`
      +`	<td>${castEmptyData(e.LICENSE)}</td>`
      +`	<td>`
          /* D : state-color
      ...
      +`</tr>`;
			...
			sample = $(sample).css('cursor', 'pointer').click(function(event) {
				if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
					...
					window.location = CTX + '/res/res_0002/detailForm?WASTE_ID=' + info.WASTE_ID;
				}
			});
