...
//			debugger;
			 ...
			 * 2019/11/08
			 * anhpv
			 * modify for moving title button to footer button and remove caption
			 * 
			 */
			var title = '<div class="footer_table_btn">';
...
//							icoCd = icoCd+' btn_orange';	//css edit
							...
							var paramsData=getGridDialogPopupParam(btn.param);	
							if(write_grant) {
								title += '<span id="' + btnId + '" class="btn ac_click sbtn basic_btn ' + icoCd + ' ' + btn.classes + '" data-func="' + btn.func + '" data-type="' + btn.type + '" data-cls="'
										+ btn.cls + '" data-gridtype="' + types + '" data-width="' + btn.width + '" data-height="' + btn.height + '"  data-url = "' + btn.url + '.' + btn.type+'"' 
										+ ' data-field="' + btn.dataField + '" data-param="'+paramsData + '"data-firstrow="' + btn.firstRow+ '" '+(btn.hidden ? 'data-hidden="true"':'')+ '>' + label + '</span>'
							}
	//					} 
			...
			var responsive = opts.gridOptions.responsive == false? '': 'responsive_table';
			$target.addClass('grid_event_apply');
			$target.append('<div id="table_' + opts.id + '" class="base_grid_table '+responsive+' group_content"> </div> ');
			$target.append(title);
...
//											eval(funcName+'(rowid,target,callback,$tr,e,this)'); 
											window[funcName](rowid,target,callback,$tr,e,this);
...
//				    	var display_title = opts.gridOptions.responsive == false? '': 'display-title'; 
//			    		return '<strong class="th-tit '+display_title+'">No </strong>' + (local.length  - index); 
...
//								var optsparam = optsparam;
								...
								return window['treeHeaderWrapper'](container,opts,model,opts.treeview,this); 
							};
...
//							templateWrapper('textField' , container, options,opts,model,funcParams,this);
				...
				case 'link':
					...
					var obj =   function (container, options) {
						var params =  funcParams ;
						var optsparam = optsparam;
						return  templateWrapper('linkField2' , container, options,opts,model,funcParams,this); 
					};
					template=obj
...
//					
				...
				case 'radio':
					...
					var obj =  function (container, options) { 
                         ...
                         */
//						return eval('checkBoxField'+'(container,options,opts,model,params,this);'  ) 
						return  templateWrapper('radioField' , container, options,opts,model,funcParams,this);
					};
					template=obj
					...
					if(gridConfig.exposeField){
						...
						// 2018.06.04 김진학 - exposeField : false 일 때, name에 일치하는 값을 찾지 못해 add 및 append 안되는 문제 수정. 
		                  ...
		                  var obj = function (container, options) {
                              ...
                              * @VIET - 보안 취약점 수정
                              */
//		                     eval('selectField'+'(container,options,opts,model,params,this);'  ) anhpv 20210108
		                     window['selectField'](container,options,opts,model,params,this);
...
//			autoFieldNameChange
			...
			var column = {
				    ...
				    media: model.media, // set media for responsive
...
//			
		...
		var drawGrid = function(opts) {
			...
			*/
			...
			var pageSize = Number(datasource.pageSize) ;
...
//				pageSize = opts.gridOptions.pageSize;
				pageSize =Number(opts.gridOptions.pageSize);
			}
			...
			//데이터 소스 정리 
			var dataSource;
			var serverPaging = opts.gridOptions.serverPaging ? opts.gridOptions.serverPaging : defaultGridOptions.serverPaging;
			if(serverPaging.enable) {
				if(opts.gridOptions.scrollable) {
					opts.gridOptions.scrollable.virtual = false;
				}
				dataSource = new kendo.data.DataSource({  
				  schema: {
				    model: opts['schemaModel'],
				    data: serverPaging.datafield,
				    total: serverPaging.totalfield
				  }//,data : local
				  ,transport: {
					read : {
						url: opts.url,
						...
						data: function () {
							var paramStringArray = opts.param.split('&');
							var paramData = {};
							$.each(paramStringArray, function(i, param) {
								var ps = param.split('=');
								paramData[decodeURI(ps[0])] = decodeURI(ps[1]);
							});
				  ...
				  ,pageSize: opts.gridOptions.serverPaging.pageSize
				  ,serverPaging: serverPaging.enable
				  ,serverFiltering: opts.gridOptions.filterable ? opts.gridOptions.filterable : defaultGridOptions.filterable
				  ,serverSorting: opts.gridOptions.sortable ? opts.gridOptions.sortable : defaultGridOptions.sortable
//						  ,change :event 
				});
			} else {
				dataSource = new kendo.data.DataSource({  
...
//	            	   optionText ='';
        		   ...
        		   $gridDom.addClass('btn-table');
        		  $gridDom.grid_callback_func(opts.boundEvent);
//         		  $gridDom.grid_callback_func(callback0);
        		  $gridDom.data('boundevent', opts.boundEvent);
        		  
        		  if(opts.gridOptions.serverPaging.enable) {
        			  setDataBinderGridWithPage($gridDom, $gridDom.getGrid().dataSource.total());
//        			  afterDrawGrid($gridDom,opts);
        		  }
...
//			}
		...
		var afterDrawGrid = function($gridDom,opts) {
			...
			if (opts.localData) {
				...
				if(opts.gridOptions.serverPaging.enable) {
					local = $gridDom.getGrid().dataSource.data();
					datas = $gridDom.getGrid().dataSource.data();
				} else {
					if(opts.url){
						$.ajax({
							url : opts.url,
...
//					opts.gridOptions.scrollable = {virtual: false }	
			...
			*/
			...
			if(datas.length <= 0){
				if($gridDom.find('.msg_empty').length<=0){
					$gridDom.find('.k-grid-content').append('<div class="msg_empty">'+msg+'</div>')
				}
			}
...
//			fix scroll by touch
			$('div.k-virtual-scrollable-wrap').unbind('touchmove');
//			wrap table content in div.table-wrap
//			$wrap = $('<div class="table-wrap"></div>');
//			$('div.k-grid-header').clone(true).appendTo($wrap);
//			$('div.k-grid-content').clone(true).appendTo($wrap);
//			$grid = $('div.k-grid-content').closest('div.base_grid_table');
//			$grid.find('div.k-grid-header').remove();
//			$grid.find('div.k-grid-content').remove();
//			$grid.find('div.table-wrap').remove();
//			$grid.prepend($wrap)
		};// end class
...
var treeset = function($gridDom){
	...
	$gridDom.find('td > span.tnode').each(function(){
		...
		$(this).closest('tr').addClass("lev"+lev);
	})
}
...
//        $(e.target).closest("tr").toggleClass("k-state-selected");
        ...
        var $grid = $(this).closest('tr').getGrid();
      	var field = $(this).attr('name');
      	var data_uid = $tr.attr('data-uid');
      	if($tr.getViewGrid().data('opts').gridOptions.selectOne){
      		$tr.getViewGrid().find('div.k-grid-content tr:not([data-uid="'+ data_uid +'"]) td input[type="checkbox"]').prop('checked', false);
      		$tr.getViewGrid().find('div.k-grid-content tr:not([data-uid="'+ data_uid +'"]) td input[type="checkbox"]').val('N');

      		
      		$grid.element.find('tr[data-uid]').each(function(i, obj){
          		var temp = $(this).getRowData($(this));
          		temp[field] = String('N') ; 
          		updateRow($(this));
          	})
  		/*var list = $grid.dataItems().toJSON();
	...
	$grid.on("change",'div.k-grid-content  td input[type="radio"].aint.toggle', function (e) {
//		alert('aaaaaaaaaaaa')
//        $(e.target).closest("tr").toggleClass("k-state-selected");
