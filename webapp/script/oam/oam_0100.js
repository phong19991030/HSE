...
function onSearch() {
	...
	// 검색 
	var data = _oam.mariaDB.getData(ctx + '/oam2/oam_0100/getData.ajax', param);
...
function makeAlarmList(data) {
	$('#alarm_list').html('');
	data.forEach((e) => {
		var row = _oam_elements.oam_0100.main.tr_alarm_row({
			id: e.EVENT_ID,
			num: e.RN,
			datetime: e.DATETIME,
			code: e.ALARM_CODE,
		...
		$('tr#tr-'+e.EVENT_ID).click(function(){
			window.location = ctx + '/oam2/oam_0100/01/eventDetail?EVENT_ID=' + $(this).prop('info').id; 
		});
	});
