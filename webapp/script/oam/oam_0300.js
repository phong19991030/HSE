 ...
 * 	input#search_criteria_b	: 검색 조건 b (작성자 이) [name="criteriaB"]
 * 	
...
function oam0300() {
	...
	$('a#register_btn').click(function(){
		window.location = ctx + '/oam2/oam_0300/reportRegister';
	});
...
function onSearch() {
	...
	// 검색 
	var data = _oam.mariaDB.getData(ctx + '/oam2/oam_0300/getData.ajax', param);
...
function makeAlarmList(data) {
	$('#row_list').html('');
	data.forEach((e) => {
		var row = _oam_elements.oam_0300.main.tr_report_row({
			ID: e.RPT_ID,
			RN: e.RN,
		...
		row = $(row).css('cursor', 'pointer').click(function(e) {
			...
			window.location = ctx + '/oam2/oam_0300/reportDetail?RPT_ID=' + $(this).prop('info').RPT_ID;
		});
		// tr 다운로드 버튼 클릭 이벤트 추가 
		row = $(row).find('.download-btn').click(function(e) {
			if($(e.target).prop('tagName') === 'I') {
				var info = $(this).parents('tr').prop('info');
				window.location.href = CTX + '/oam2/oam_0300/downloadPDF.ajax?RPT_ID=' + info.RPT_ID;
			}
		}).parents('tr');
		// 추가
		$('#row_list').append(row);
	});
	if(data.length === 0) $('#row_list').append(_oam_elements.oam_0300.main.no_data());
}
