...
(function(window){
	...
	Search.prototype.getParam = function() {
		...
		if(this.A) param.SEARCH_ROLE_ID = this.A;
...
function project0100() {
	...
	$('a#REGISTER_BTN').click(function(){
		window.location = ctx + '/project/project_0100/registerForm';
	});
...
function onSearch() {
	...
	// 검색
	var data = _sys.mariaDB.getData(CTX + '/project/project_0100/getData.ajax', param);
	console.log(data);
...
function makeList(data) {
	...
	list.forEach((e) => {
		...
		var sample = 
			...
			+ '	<td>' + e.START_TIME_PROJECT + '</td>'
			+ '	<td>' + e.END_TIME_PROJECT + '</td>'
			+ '	<td>' + e.MANAGER + '</td>'
			+ '	<td>' + e.TOTAL_MANPOWER + '</td>'
			+ '	<td>' + e.CONTRACT_COMM_NM + '</td>'
			+ '	<td>' + e.INIT_COMM_NM + '</td>'
			+ '	<td>' + e.IMPLEMENT_COMM_NM + '</td>'
			+ '	<td>' + e.CLOSED_COMM_NM + '</td>'
			+ '	<td>' + e.DETAIL_CONTENT + '</td>'
			+ '</tr>';
		...
		sample = $(sample).css('cursor', 'pointer').click(function(event) {
			if(event.target.tagName === 'TD' || event.target.tagName === 'EM' || event.target.tagName === 'SPAN') {
				...
				window.location = CTX + '/project/project_0100/detailForm?PROJECT_ID=' + info.PROJECT_ID;
			}
		});
