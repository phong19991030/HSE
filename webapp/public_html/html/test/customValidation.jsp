 ...
 <!-- 발전기 등록 -->
    <div class="system-detail-wrap">
      <div class="system-left">
        ...
        <div class="tit-wrap">
          <h2 class="heading3">
            <span class="txt">Custom Validation</span>
            <!-- <span class="version">V47</span> -->
          </h2>
          <ul class="location">
            <li>Components</li>
            <li class="bold">Custom Validation</li>
          </ul>
        </div>
        <!--//tit-wrap-->
        <!-- registration form -->
        <pre>
&lt!-- you can add a function by input's event (keyup, keydown, onclick, onchange...) to check the value of input.  --&gt
&lt!-- After that, you can add validation message by  
$inputElement.inputWarning('message here') for error or $inputElement.available() for success --&gt

&lt!-- For example: --&gt
&ltscript&gt

	function checkInputOnChange(){
		if($('#test1').val() == 'nova'){
			$('#test1').available();
		}else{
			$('#test1').inputWarning('Input 1 must be "nova"!');
		}
	}
	
	function checkInputKeyUp(){
		if($('#test2').val().length == 8){
			$('#test2').available();
		}else{
			$('#test2').inputWarning('Input 2 must be 8 character!');
		}
	}
...
&lt/script&gt
  &ltul class="registration-form-lst"&gt
	              ...
	              &ltli&gt
	              &ltspan  class="essential"&gtEvent on change&lt/span&gt
	              &ltdiv class="registration-write"&gt
	                &ltdiv class="input-group"&gt
	                  &ltlabel for="wtgId" class="sr-only"&gtWTG ID&lt/label&gt
	                  &ltinput type="text" onchange="checkInputOnChange()" name="test1" id="test1" placeholder=""&gt
	                &lt/div&gt
	              &lt/div&gt
	            &lt/li&gt
	            
	              &ltli&gt
	              &ltspan  class="essential"&gtEvent on key up&lt/span&gt
	              &ltdiv class="registration-write"&gt
	                &ltdiv class="input-group"&gt
	                  &ltlabel for="wtgId" class="sr-only"&gtWTG ID&lt/label&gt
	                  &ltinput type="text" onkeyup="checkInputKeyUp()" name="test2" id="test2" placeholder=""&gt
	                &lt/div&gt
	              &lt/div&gt
	            &lt/li&gt
	          &lt/ul&gt
          ...
          <div class="registration-form registration-form1">
	        <div class="registration-form-lst-wrap">
	          <ul class="registration-form-lst">
	              ...
	              <li>
	              <span  class="essential">Event on change</span>
	              <div class="registration-write">
	                <div class="input-group">
	                  <label for="wtgId" class="sr-only">WTG ID</label>
	                  <input type="text" onchange="checkInputOnChange()" name="test1" id="test1" placeholder="">
	                </div>
	              </div>
	            </li>
	            
	              <li>
	              <span  class="essential">Event on key up</span>
	              <div class="registration-write">
	                <div class="input-group">
	                  <label for="wtgId" class="sr-only">WTG ID</label>
	                  <input type="text" onkeyup="checkInputKeyUp()" name="test2" id="test2" placeholder="">
	                </div>
	              </div>
	            </li>
	          </ul>
